...
import BoosterActivities from "../../../components/layouts/BoosterActivities";
...
function Likes() {
  useEffect(() => {
    ...
    if (localStorage.getItem("lastlikedTabDashboard")) {
      const config = {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("access_token"),
        },
      };
      setSelectedTab(localStorage.getItem("lastlikedTabDashboard"));
      axios.post(`${domain}/api/pilot/getLikedMedia`, config).then((res) => {
        console.log(res.data);
        let Images = res.data.filter(
          (data) =>
            data.fileType == localStorage.getItem("lastlikedTabDashboard")
        );
        if (Images.length == 0) {
    ...
    setTimeout(() => {
      localStorage.removeItem("lastlikedTabDashboard");
    }, 2000);
  }, []);
  ...
  let openImageView = (id) => {
    ...
    localStorage.setItem("lastlikedTabDashboard", selectedTab);
  };
    ...
    <div>
      ...
      <center>
        <div id="mediaToHide" style={{ display: "none" }}>
          <div><Alert severity="info">No Liked Media <Link href = "/"><a style = {{textDecoration: "underline"}}>Browse</a></Link> now</Alert></div>
        </div>
      </center>
          ...
          return (
            <Grid item xl ={4} lg={4} md={6} sm={6} xs={12} key={i}>
              <div style={{ position: "relative", height: "100%" }}>
                ...
                <div className={styles.homepageUserContainer} style = {{bottom: "0px", borderRadius: "5px"}}>
                  <span
...
Likes.Layout = BoosterActivities;
