...
function CreateJob() {
  ...
  let [activeJobs1, setActiveJobs1] = useState(0)
  ...
  useEffect(() => {
    ...
    axios
        ...
        .then((res) => {
          console.log(res)
          setActiveJobs1(res.data.activeJobs)
    ...
    axios
      .post(`${domain}/api/draftJob/getDetails`, {
        draftId: id,
      })
      .then((res) => {
        console.log(res);
  ...
  let CreateJob = () => {
    ...
    if ((desc.length < 50 || desc.length > 50000)) {
      document.getElementById("description_error").innerHTML =
        "Please write atleast 50 to maximum 1000 words";
    ...
    if (focusField == "") {
      if(activeJobs1 >= mySubscription.activeJobs){
        //popup goes here
        setOpen(true)
      }else{
        const config = {
  ...
  let cancelDraft = () =>{
    localStorage.setItem("cancelDraft", "true")
    ...
    <>
      ...
      >
        <div className={job.container}>
          ...
          <div className={job.jobMainTitle}>Complete your draft job</div>
          ...
          <div style={{ display: "flex" }}>
            <button className={job.jobCancel} onClick={cancelDraft}>Cancel</button>
      ...
      >
        <div className="popupContainer">
          <ClearRoundedIcon className="popupClose" onClick={()=>setOpen(false)} />
          <div className="popupTitle">
            You have Exceeded the Limit !! Upgrade to continue
          </div>
          <center>
      ...
      >
        <div className="popupContainer">
          <ClearRoundedIcon className="popupClose" onClick={()=>setLogin(false)} />
          <div className="popupTitle">
