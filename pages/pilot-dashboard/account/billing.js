...
function Billing() {
  ...
  useEffect(() => {
    ...
    axios.post(`${domain}/api/pilot/sendBillingAddress`, config).then((res) => {
      console.log(res.data);
      setData({
        ...data,
        line1: res.data.line1 ? res.data.line1 : "",
        line2: res.data.line2 ? res.data.line2 : "",
        postalCode: res.data.postal_code ? Number(res.data.postal_code) : "",
  ...
  let saveDetails = () => {
    ...
    if (
      ...
      document.getElementById("postalCode_error").innerHTML =
        "Postal Address should be max 10 numbers";
