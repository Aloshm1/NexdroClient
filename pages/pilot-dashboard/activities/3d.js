...
function Index() {
  ...
  let getApprovedImages = () => {
    ...
    axios.post(`${domain}/api/rearrange/getRearrangedMedia`,{fileType: "3d"}, config)
    .then((res) => {
      console.log(res)
  ...
  useEffect(() => {
    ...
    axios.post(`${domain}/api/image/getRejected3d`, config).then((res) => {
  ...
  const saveRearrange = () => {
    ...
    .then((res) => {
      console.log(res)
      if (res.data === "No file available"){
        axios.post(`${domain}/api/rearrange/createRearrange`, {fileType: "3d", media: media}, config)
        .then(res => {
        ...
        axios.post(`${domain}/api/rearrange/updateRearrangeFiles`, {fileType: "3d", media: media}, config)
        .then(res => {
          setApprovedImages(rearrangedFiles)
          cancelRearrange()
          console.log(res)
      ...
      {tab == "approved" ? (
        ...
        /></div>
          ...
          {approvedImages.length === 0 ? (
            <Alert severity="info">No approved 3d Yet</Alert>
          ) : (
            <></>
          )}
      ...
      {tab == "pending" ? (
        <div style={{ marginTop: "20px" }}>
          {pendingImages.length === 0 ? (
            <center>
              <Alert severity="info">No pending 3d Yet</Alert>
            </center>
          ) : (
            <></>
          )}
      ...
      {tab == "rejected" ? (
        <div style={{ marginTop: "20px" }}>
          {rejectedImages.length === 0 ? (
            <Alert severity="success">No rejected 3d Yet</Alert>
          ) : (
            <></>
          )}
          <Grid container spacing={2}>
